module ActiveRecord
  module CustomNestedAttributes #:nodoc:
    ...
    module ClassMethods
      ...
      def custom_accepts_nested_attributes_for(*attr_names)
        options = { :key => 'id', :allow_destroy => false }
        options.update(attr_names.extract_options!)
        options.assert_valid_keys(:key, :allow_destroy, :reject_if)
        ...
        attr_names.each do |association_name|
          if reflection = reflect_on_association(association_name)
            ...
            #   custom_assign_nested_attributes_for_one_to_one_association(:pirate, attributes, 'name', false)
            # end
            class_eval %{
              def #{association_name}_attributes=(attributes)
                custom_assign_nested_attributes_for_#{type}_association(:#{association_name}, attributes, '#{options[:key]}', #{options[:allow_destroy]})
              end
            }, __FILE__, __LINE__
          else
    ...
    def custom_assign_nested_attributes_for_one_to_one_association(association_name, attributes, key, allow_destroy)
      ...
      if attributes['id'].blank?
        unless reject_new_record?(association_name, attributes)
          ...
          elsif existing_record = klass.send("find_by_#{key}", attributes[key])
            existing_record.attributes = attributes.except(*UNASSIGNABLE_KEYS)
            send("#{association_name}=", existing_record)
          else
    ...
    #     { :id => '2', :_delete => true }
    #   ])
    def custom_assign_nested_attributes_for_collection_association(association_name, attributes_collection, allow_destroy)
